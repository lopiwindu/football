...
import { User, Mail, Wallet, Bell, Shield, DollarSign, CheckCircle } from 'lucide-react';
...
export function ProfileSettings({ balance, onBalanceUpdate, predictions }: ProfileSettingsProps): JSX.Element {
  ...
  const wonBets = predictions.filter((p: UserPrediction) => p.status === 'won').length;
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card className="bg-slate-800 border-slate-700">
          <CardHeader className="pb-2">
            <CardDescription className="text-slate-400 text-xs">Total Bets</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardHeader className="pb-2">
            <CardDescription className="text-slate-400 text-xs">Total Wagered</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold text-white">${totalWagered.toFixed(0)}</p>
          </CardContent>
        </Card>

        <Card className="bg-slate-800 border-slate-700">
          <CardHeader className="pb-2">
            <CardDescription className="text-slate-400 text-xs">Wins</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardHeader className="pb-2">
            <CardDescription className="text-slate-400 text-xs">Win Rate</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold text-blue-400">{winRate.toFixed(0)}%</p>
          </CardContent>
        </Card>
      </div>
            ...
            <TabsContent value="profile" className="space-y-4">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-400">Connected via MetaMask</p>
                </div>
                ...
                <div className="flex items-center justify-between p-4 bg-slate-700/50 border border-slate-600 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Receive updates on your predictions</p>
                    </div>
                  </div>
                  <Switch
                ...
                <div className="flex items-center justify-between p-4 bg-slate-700/50 border border-slate-600 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Automatically settle winning bets</p>
                    </div>
                  </div>
                  <Switch
                    checked={autoSettle}
